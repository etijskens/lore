...
class CommandBase:
    ...
    def execute_repeat(self,attempts=6,wait=60,check=True,stderr_is_error=False,error_log=None, post_processor=None,verbose=False,timeout=None):
        ...
        while attempts_left:
            ...
            raise lore.exceptions.RepeatedExecutionFailed('\n'+self.repeat_messages)
    ...
    def process_output(self, post_processor=None
        ...
        if check:
            if self.result.returncode!=0:
                ...
                raise lore.exceptions.NonZeroReturnCode(msg, result=self.result, error_log=error_log)
        
        if stderr_is_error:
            if self.result.stderr:
                msg = f"\n  {type(self).__name__} '{self.command}'\n  yields output on stderr: \n{self.result.stderr}"
                raise lore.exceptions.Stderr(msg, result=self.result, error_log=error_log)
